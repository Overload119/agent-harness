...
1. Stay inside the current repo checkout.
   ...
   - Treat the run state file path as an opaque handle that may only be passed back to `.agent-harness/bin/ah-run-state update --run-file ...`.
2. Use CLI commands for shared run persistence.
   - Use `.agent-harness/bin/ah-run-state update --run-file ...` to report phase or progress messages when helpful.
...
2. Report start via `.agent-harness/bin/ah-run-state update`.
...
6. Report final phase/message via `.agent-harness/bin/ah-run-state update`.
