...
3. Work on one task only.
   ...
   - Treat enriched task metadata such as `dependsOn`, `implementationScope`, `implementationPlan`, `verification`, `handoff`, and `gotchas` as the primary execution brief when present.
   - Prefer the assigned task's structured metadata over broad repo-wide rediscovery.
...
5. Verification matters.
   ...
   - Use task-level verification metadata when it exists.
...
1. Read the PRD, inspect the assigned task, and start from any structured task metadata before doing extra repo exploration.
